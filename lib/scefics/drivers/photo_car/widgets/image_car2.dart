...
class _PhotoCarPhoto2State extends ConsumerState<PhotoCarPhoto2> {
      ...
      appBar: AppBar(
        ...
        actions: [
          GestureDetector(
            onTap: () {
              try {
                ref.read(controllerCarPhoto6.notifier).file2.path.length > 10
          ...
          child: Center(
            child: Column(
              ...
              children: [
                GestureDetector(
                  onTap: () {
                    ref.read(controllerCarPhoto6.notifier).getImageCamera(1);
                  },
                  child: Container(
                    ...
                    child: getDataFile() == "1"
                        ? Image.file(ref.watch(controllerCarPhoto6.notifier).file2)
                ...
                MaterialButton(
                  onPressed: () {
                    ref.read(controllerCarPhoto6.notifier).getImage(1);
                  },
                  height: 55,
                  minWidth: double.infinity,
                  color: AppColors.colorBackground,
  ...
  String getDataFile(){
    try{
      return ref.watch(controllerCarPhoto6.notifier).file2.path.length >= 10?"1":"0";
    }catch(e){
      return "0";
    }
  }
